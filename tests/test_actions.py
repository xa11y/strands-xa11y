...
def test_wait_polls_through_the_locator(editor, calls):
    result = run(
        WaitAction(
            type="wait", target=ElementTarget(app="TextEdit", selector="button[name='Bold']"), condition="visible"
        )
    )
    assert result["status"] == "success"
    assert called(calls, "wait_visible")
